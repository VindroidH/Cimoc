...
public class GuFeng extends MangaParser {
    ...
    public Request getSearchRequest(String keyword, int page) throws UnsupportedEncodingException {
        String url = "";
        if (page == 1) {
            url = StringUtils.format("https://m.gufengmanhua.com/search/?keywords=%s",
    ...
    public SearchIterator getSearchIterator(String html, int page) {
        ...
        return new NodeIterator(body.list("div.UpdateList > div.itemBox")) {
            ...
            protected Comic parse(Node node) {
                ...
                String cid = node.attr("div.itemTxt > a", "href").replace("https://m.gufengmanhua.com/manhua/", "");
    ...
    public Request getInfoRequest(String cid) {
        String url = "https://m.gufengmanhua.com/manhua/".concat(cid) + "/";
        return new Request.Builder()
                .addHeader("User-Agent", "Mozilla/5.0 (iPhone; CPU iPhone OS 12_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/12.0 Mobile/15A372 Safari/604.1")
                .url(url)
                .build();
    ...
    public Request getImagesRequest(String cid, String path) {
        String url = StringUtils.format("https://www.gufengmanhua.com/manhua/%s/%s.html", cid, path);
        return new Request.Builder()
                .addHeader("User-Agent", "Mozilla/5.0 (iPhone; CPU iPhone OS 12_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/12.0 Mobile/15A372 Safari/604.1")
                .url(url)
                .build();
        ...
        if (str != null) {
            try {
                ...
                for (int i = 0; i != array.length; ++i) {
                    // 去掉首末两端的双引号和反斜杠
                    String s = array[i].substring(1, array[i].length() - 1).replace("\\", "");
                    Long comicChapter = chapter.getId();
                    Long id = Long.parseLong(comicChapter + "000" + i);
                    if (!s.toLowerCase().startsWith("http")) {
                        s = "https://res5.gufengmanhua.com" + s;
                    }
                    list.add(new ImageUrl(id, comicChapter, i + 1, s, false));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
